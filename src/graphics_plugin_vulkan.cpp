...
namespace {
    ...
    [[noreturn]] inline void ThrowVkResult(VkResult res, const char *originator = nullptr,
...
#define CHECK_VKRESULT(res, cmdStr) CheckVkResult(res, cmdStr, FILE_AND_LINE);
    ...
    struct CmdBuffer {
        enum class CmdBufferState {
            Undefined, Initialized, Recording, Executable, Executing
        };
        ...
        static std::string StateString(CmdBufferState s) {
            switch (s) {
...
#define CHECK_CBSTATE(s)                                                                                           \
    do                                                                                                             \
        if (state != (s)) {                                                                                        \
            LOG_ERROR("Expecting state %s, but it is %s", StateString(s).c_str(), StateString(state).c_str());     \
            return false;                                                                                          \
        }                                                                                                          \
    while (0)
        ...
        bool Init(const VulkanDebugObjectNamer &namer, VkDevice device, uint32_t queueFamilyIndex) {
            ...
            CHECK_VKCMD(
                    namer.SetName(VK_OBJECT_TYPE_FENCE, (uint64_t) execFence, "hello_xr fence"))
        ...
        bool Wait() {
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                LOG_INFO("Waiting for CmdBuffer fence timed out, retrying...");
            }
...
#undef LIST_CMDBUFFER_STATES
    ...
    private:
        ...
        void Load(uint32_t index, const std::vector<uint32_t> &code) {
            ...
            LOG_INFO("Loaded %s shader", name.c_str());
        }
    };
...
// RenderPass wrapper
    struct RenderPass {
        ...
        bool Create(const VulkanDebugObjectNamer &namer, VkDevice device, VkFormat aColorFmt,
                    VkFormat aDepthFmt) {
            m_vkDevice = device;
            colorFmt = aColorFmt;
            depthFmt = aDepthFmt;
...
// VkImage + framebuffer wrapper
    struct RenderTarget {
        ...
        void Create(const VulkanDebugObjectNamer &namer, VkDevice device, VkImage aColorImage,
                    VkImage aDepthImage, VkExtent2D size,
...
// Pipeline wrapper for rendering pipeline state
    ...
    struct DepthBuffer {
        ...
        void
        Create(const VulkanDebugObjectNamer &namer, VkDevice device, MemoryAllocator *memAllocator,
    ...
    struct SwapchainImageContext {
        explicit SwapchainImageContext(XrStructureType _swapchainImageType) : swapchainImageType(
                _swapchainImageType) {}
        ...
        Create(const VulkanDebugObjectNamer &namer, VkDevice device,
            ...
            auto colorFormat = (VkFormat) swapchainCreateInfo.format;
        ...
        void BindRenderTarget(uint32_t index, VkRenderPassBeginInfo *renderPassBeginInfo) {
            if (renderTarget[index].fb == VK_NULL_HANDLE) {
                renderTarget[index].Create(m_namer, m_vkDevice, swapchainImages[index].image,
    ...
    struct VulkanGraphicsPlugin : public IGraphicsPlugin {
        explicit VulkanGraphicsPlugin(const std::shared_ptr<Options> &options)
        ...
        void InitializeDevice(XrInstance instance, XrSystemId systemId) override {
            ...
            CHECK_XRCMD(
                    GetVulkanGraphicsRequirements2KHR(instance, systemId, &graphicsRequirements))
            ...
            if (validationLayerName) {
                ...
                LOG_WARN("No validation layers found in the system, skipping");
            }
#endif
            ...
            std::vector<const char *> extensions;
            {
                ...
                CHECK_VKCMD(vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount,
            ...
            appInfo.pApplicationName = "but_telepresence";
            appInfo.applicationVersion = 1;
            appInfo.pEngineName = "but_telepresence";
            ...
            if (vkCreateDebugUtilsMessengerEXT != nullptr &&
                ...
                CHECK_VKCMD(vkCreateDebugUtilsMessengerEXT(m_vkInstance, &debugInfo, nullptr,
            ...
            deviceInfo.ppEnabledExtensionNames = deviceExtensions.empty() ? nullptr
...
#include "frag.spv"
            ...
            CHECK_VKCMD(m_namer.SetName(VK_OBJECT_TYPE_SEMAPHORE, (uint64_t) m_vkDrawDone,
            ...
            m_drawBuffer.Init(m_vkDevice, &m_memAllocator,
                              {{0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Geometry::Vertex,
                                                                           Position)},
                               {1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Geometry::Vertex,
                                                                           Color)}});
        ...
        void RenderView(const XrCompositionLayerProjectionView &layerView,
            ...
            swapchainContext->depthBuffer.TransitionLayout(&m_cmdBuffer,
        ...
        uint32_t GetSupportedSwapchainSampleCount(
                const XrViewConfigurationView &) override { return VK_SAMPLE_COUNT_1_BIT; }

        void UpdateOptions(
                const std::shared_ptr<Options> &options) override { m_clearColor = options->GetBackgroundClearColor(); }
...
#define MK_OBJECT_TYPE_CASE(name) \
    case VK_OBJECT_TYPE_##name:   \
        ...
        static VkBool32 debugMessage(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
            ...
            LOG_INFO("%s (%s 0x%lx) %s", flagNames.c_str(), objName.c_str(), object,
        ...
        debugMessageThunk(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
                          ...
                          const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData,
                          void *pUserData) {
            return VulkanGraphicsPlugin::debugMessage(messageSeverity, messageTypes, pCallbackData);
        }

        [[nodiscard]] static XrStructureType
        GetGraphicsBindingType() { return XR_TYPE_GRAPHICS_BINDING_VULKAN2_KHR; }

        [[nodiscard]] static XrStructureType
        GetSwapchainImageType() { return XR_TYPE_SWAPCHAIN_IMAGE_VULKAN2_KHR; }

        virtual XrResult CreateVulkanInstanceKHR(XrInstance instance,
        ...
        virtual XrResult
        CreateVulkanDeviceKHR(XrInstance instance, const XrVulkanDeviceCreateInfoKHR *createInfo,
        ...
        virtual XrResult GetVulkanGraphicsDevice2KHR(XrInstance instance,
