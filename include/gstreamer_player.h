...
private:
    ...
    //const std::string h264Pipeline_ = "udpsrc name=udpsrc buffer-size=10000 ! capsfilter name=rtp_capsfilter caps=\"application/x-rtp, encoding-name=H264, media=video, clock-rate=90000, payload=96\" ! identity name=udpsrc_ident ! rtph264depay ! identity name=rtpdepay_ident silent=false ! h264parse ! video/x-h264, stream-format=byte-stream, alignment=au ! queue max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! amcviddec-omxqcomvideodecoderavc ! glupload ! glcolorconvert ! gldownload ! video/x-raw,format=I420 ! identity name=dec_ident ! identity name=queue_ident silent=false ! appsink emit-signals=true name=appsink sync=false";
    //const std::string h264Pipeline_ =   "udpsrc name=udpsrc buffer-size=10000 ! capsfilter name=rtp_capsfilter caps=\"application/x-rtp, encoding-name=H264, media=video, clock-rate=90000, payload=96\" ! identity name=udpsrc_ident ! rtpjitterbuffer latency=50 do-lost=true drop-on-latency=true do-retransmission=false ! rtph264depay ! identity name=rtpdepay_ident ! h264parse config-interval=-1 ! video/x-h264, stream-format=byte-stream, alignment=au, parsed=true ! amcviddec-omxqcomvideodecoderavc ! glupload ! glcolorconvert ! glimagesink sync=false";
    const std::string h264Pipeline_ =   "udpsrc name=udpsrc buffer-size=10000 ! capsfilter name=rtp_capsfilter caps=\"application/x-rtp, encoding-name=H264, media=video, clock-rate=90000, payload=96\" ! identity name=udpsrc_ident ! rtpjitterbuffer latency=50 do-lost=true drop-on-latency=true do-retransmission=false ! rtph264depay ! identity name=rtpdepay_ident ! h264parse config-interval=-1 ! amcviddec-omxqcomvideodecoderavc ! glimagesinkelement name=appsink sync=false";
    const std::string h265Pipeline_ =   "udpsrc name=udpsrc buffer-size=10000 ! capsfilter name=rtp_capsfilter caps=\"application/x-rtp, encoding-name=H265, media=video, clock-rate=90000, payload=96\" ! identity name=udpsrc_ident ! rtpjitterbuffer latency=50 do-lost=true drop-on-latency=true do-retransmission=false ! rtph265depay ! identity name=rtpdepay_ident ! h265parse config-interval=-1 ! amcviddec-omxqcomvideodecoderhevc ! glimagesinkelement name=appsink sync=false";
